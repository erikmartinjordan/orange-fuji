...
function render(state) {
  ...
  if (state.licensed) {
    ...
    show('success');
    setTimeout(() => window.pico.closeWindow(), 1500);
    return;
  }
...
async function load() {
  try {
    render(await window.pico.getLicenseState());
  } catch (_) {
    setMsg('Could not load license status.', 'error');
  }
}

el.buy?.addEventListener('click', () => window.pico.openBuyLicense());
...
el.activateBtn?.addEventListener('click', async () => {
  ...
  try {
    const state = await window.pico.activateLicense(email);
    render(state);
    if (state?.licensed) window.pico.closeWindow();
  } catch (error) {
